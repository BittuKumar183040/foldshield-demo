...
const ParticleGrid: React.FC<ParticleGridProps> = ({
  ...
  useEffect(() => {
    // detect Tailwind dark mode (class strategy)
    const checkDark = () => {
  ...
  useEffect(() => {
    ...
    const effectiveBg = isDark ? "#000000" : bgColor;
    const effectiveParticle = isDark ? "255,255,255" : particleColor;
    ...
    const draw = () => {
      ...
      dots.forEach((d) => {
        ...
        ctx.fillStyle = `rgba(${effectiveParticle}, ${alpha})`;
        ctx.fill();
      });
  ...
  }, [spacing, radius, strength, bgColor, particleColor, isDark]);
