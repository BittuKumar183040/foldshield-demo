...
const Slider = ({ data, active, setActive }: SliderProps) => {
  ...
  const isVideo = (src: string) => {
    return /\.(mp4|mkv|webm|ogg)$/i.test(src);
  };
  ...
  return (
    <div className="absolute top-0 left-0 h-full w-full flex-1 overflow-hidden">
      ...
      {data.length > 1 && 
        <div className="absolute bottom-6 left-0 px-2 right-6 z-10 flex gap-4 w-full">
          {data.map((item, i) => (
            <div
              key={i}
              onClick={() => setActive(i)}
            ...
            >
              ...
              <div className="flex flex-col">
                <p
                  className={` text-white text-sm lg:block hidden transition whitespace-nowrap overflow-hidden text-ellipsis ${active === i ? "text-cyan-400 opacity-100" : "text-white/60"}`}
                >
                  {item.title}
                </p>
                {item.desc && (
